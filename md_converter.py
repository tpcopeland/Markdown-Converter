...
- XLSX: Excel spreadsheet from markdown tables (optional dependency)
...
- Chapter headings: caps at H6 (maximum valid HTML heading level)
...
def sanitize_filename_for_format(name: str, extension: str) -> str:
    ...
    for ext in ['.html', '.htm', '.md', '.markdown', '.docx', '.doc', '.xlsx', '.xls']:
...
XLSX_COLORS = {
    "very_high": "90EE90",      # Light green (lime green)
    "high": "C6EFCE",           # Lighter green (pale green)
    "header_bg": "4472C4",      # Blue header background
    "header_font": "FFFFFF",    # White header text
}


def check_xlsx_dependencies() -> Tuple[bool, str]:
    """
    Check if XLSX conversion dependencies are available.

    Returns:
        Tuple of (is_available, error_message)
    """
    if not HAS_OPENPYXL:
        return False, "openpyxl is not installed. Install with: pip install openpyxl"
    return True, ""


def split_markdown_table_row(line: str) -> List[str]:
    """
    Split a markdown table row by pipes, respecting escaped pipes.
    
    Args:
        line: The raw table line
        
    Returns:
        List of raw cell contents (including empty start/end if present)
    """
    parts = line.split('|')
    result = []
    buffer = ""
    ...
    for part in parts:
        ...
        # Count trailing backslashes to check if the pipe was escaped
        backslashes = 0
        ...
        if backslashes % 2 == 1:
            # Odd number of backslashes means the pipe was escaped
            # Keep in buffer and continue to next part
            continue
        else:
            # Even number of backslashes (or 0) means real separator
...
def unescape_table_cell(cell: str) -> str:
    """
    Unescape a markdown table cell value.

    Converts escaped characters back to their literal form:
    - \\| becomes |
    - \\\\ becomes \\

    Args:
        cell: The raw cell content with escape sequences
    ...
    result = cell.replace("\\\\", placeholder)
...
def parse_markdown_tables(md_content: str) -> List[Tuple[List[str], List[List[str]]]]:
    ...
    Returns:
        List of (headers, rows) tuples for each table found.
    """
    tables = []
    ...
    while i < len(lines):
        ...
        # Look for start of a table (line starting with |)
        if line.startswith('|'):
            table_lines = [line]
            i += 1
            ...
            # Parse this table if it has at least header, separator, and data
            if len(table_lines) >= 3:
                # Parse header row
                header_line = table_lines[0]
                
                raw_headers = split_markdown_table_row(header_line)
                # Remove first and last empty strings if they exist (standard markdown table)
                if len(raw_headers) >= 2 and raw_headers[0].strip() == '' and raw_headers[-1].strip() == '':
                    raw_headers = raw_headers[1:-1]
                
                headers = [unescape_table_cell(cell.strip()) for cell in raw_headers]

                # Skip separator row (index 1), parse data rows
                rows = []
                for tline in table_lines[2:]:
                    raw_cells = split_markdown_table_row(tline)
                    if len(raw_cells) >= 2 and raw_cells[0].strip() == '' and raw_cells[-1].strip() == '':
                        raw_cells = raw_cells[1:-1]

                    cells = [unescape_table_cell(cell.strip()) for cell in raw_cells]
...
def _find_relevance_column(headers: List[str]) -> int:
    """Find the index of the Relevance column (0-indexed)."""
    for i, header in enumerate(headers):
        if header.lower() == 'relevance':
            return i
    return -1


def _get_relevance_fill(relevance: str):
    """Return the appropriate fill color based on relevance level."""
    if not HAS_OPENPYXL:
        return None

    relevance_upper = relevance.upper().strip()

    if "VERY HIGH" in relevance_upper:
        return PatternFill(start_color=XLSX_COLORS["very_high"],
                          end_color=XLSX_COLORS["very_high"],
                          fill_type="solid")
    elif relevance_upper.startswith("HIGH"):
        # Match "HIGH" but not "MEDIUM-HIGH" etc.
        return PatternFill(start_color=XLSX_COLORS["high"],
                          end_color=XLSX_COLORS["high"],
                          fill_type="solid")
    return None
...
def convert_markdown_to_xlsx(
    ...
    Args:
        markdown_content: The markdown text containing tables
        output_path: Optional path to write the XLSX file
        sheet_name: Name for the worksheet
        highlight_relevance: Apply green highlighting to HIGH/VERY HIGH relevance rows
        auto_filter: Add auto-filter dropdowns to header row
    ...
    Raises:
        ...
        ValueError: If no valid tables found in markdown
    """
    available, error = check_xlsx_dependencies()
    if not available:
        raise ImportError(error)

    # Parse all tables from markdown
    tables = parse_markdown_tables(markdown_content)

    if not tables:
        raise ValueError("No valid markdown tables found in the content")

    # Use the first table (or could be modified to handle multiple)
    ...
    header_fill = PatternFill(start_color=XLSX_COLORS["header_bg"],
                              end_color=XLSX_COLORS["header_bg"],
                              fill_type="solid")
    header_font = Font(bold=True, color=XLSX_COLORS["header_font"])
    thin_border = Border(
    ...
    # Find relevance column index
    relevance_col = _find_relevance_column(headers) if highlight_relevance else -1
    ...
    for row_idx, row_data in enumerate(rows, start=2):
        # Determine row fill based on relevance
        row_fill = None
        if relevance_col >= 0 and relevance_col < len(row_data):
            row_fill = _get_relevance_fill(row_data[relevance_col])
        ...
        for col_idx, value in enumerate(row_data, start=1):
            ...
            # Apply row highlighting if applicable
            if row_fill:
                cell.fill = row_fill

    # Auto-fit column widths (with max width limit)
    for col_idx, header in enumerate(headers, start=1):
...
def get_table_preview(markdown_content: str) -> Optional[Tuple[List[str], int]]:
    """
    Get a preview of tables in the markdown content.

    Returns:
        Tuple of (headers, row_count) for the first table, or None if no tables found.
    """
    tables = parse_markdown_tables(markdown_content)
    if tables:
        headers, rows = tables[0]
...
with st.container(border=True):
    ...
    elif export_format == "XLSX (Excel)" and md_text:
        # Show table preview
        table_info = get_table_preview(md_text) if xlsx_available else None
        if table_info:
            headers, row_count = table_info
            st.success(f"Found table with {len(headers)} columns and {row_count} rows")
            with st.expander("Table columns"):
                st.write(", ".join(headers))
        else:
            st.warning("No markdown tables found in the content. XLSX export requires at least one table.")
...
with st.container(border=True):
    ...
    if export_format == "HTML (Offline)":
        ...
        # ----- XLSX OPTIONS -----
        st.markdown("**Spreadsheet Settings**")
        col1, col2 = st.columns([1, 1], gap="medium")
        with col1:
        ...
        with col2:
            xlsx_freeze_header = st.toggle("Freeze header row", value=True, help="Keep header row visible when scrolling")
            xlsx_highlight_relevance = st.toggle(
                "Highlight relevance levels",
                value=True,
                help="Apply green highlighting to HIGH/VERY HIGH relevance rows (if Relevance column exists)"
            )

        st.divider()

        st.markdown("**Column Settings**")
        xlsx_max_width = st.slider(
            "Maximum column width",
            min_value=20,
            max_value=100,
            value=50,
            help="Maximum width for auto-sized columns"
        )

        # Set defaults for HTML/DOCX options (not used but needed for variable scope)
        toc_mode = "none"
        toc_levels = "h2"
        collapsible_mode = "none"
        theme_preset_value = "default"
...
with build_col:
        ...
        if not md_text.strip():
            ...
            try:
                ...
                if export_format == "HTML (Offline)":
                    ...
                    xlsx_bytes = convert_markdown_to_xlsx(
                        md_text,
                        sheet_name=xlsx_sheet_name,
                        highlight_relevance=xlsx_highlight_relevance,
                        auto_filter=xlsx_auto_filter,
                        freeze_header=xlsx_freeze_header,
                        max_col_width=xlsx_max_width
                    )
                    ...
                    # Store result in session state
                    st.session_state["generated_xlsx"] = xlsx_bytes
                    st.session_state["generated_xlsx_name"] = default_name
                    st.session_state["generated_html"] = None
                    st.session_state["last_html"] = None
                    st.session_state["generated_docx"] = None
                    st.session_state["generated_pdf"] = None
    ...
    if st.session_state.get("generated_xlsx"):
        st.download_button(
            ...
            file_name=st.session_state.get("generated_xlsx_name", "document.xlsx"),
